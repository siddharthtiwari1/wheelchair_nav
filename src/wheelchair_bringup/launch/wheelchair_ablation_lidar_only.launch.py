...
"""
ABLATION: LIDAR-ONLY NAVIGATION (no STVL, no PRISM scan fusion)
================================================================
Baseline for PRISM paper ablation study.

- AMCL localizes on /scan_lidar (footprint-filtered LiDAR only)
- Local costmap: obstacle_layer + inflation_layer (NO stvl_layer)
- Map: lidar-only SLAM map
- scan_lidar node applies wheelchair footprint filter to /scan_filtered
- Cameras still launch (via localization sub-launch) but are NOT used by Nav2

Usage:
    ros2 launch wheelchair_bringup wheelchair_ablation_lidar_only.launch.py
...
def generate_launch_description():
    ...
    # ABLATION DEFAULTS — LiDAR-only map + LiDAR-only nav2 params
    default_map_file = os.path.join(ws_root, 'maps', '2026-03-06_lidar_only.yaml')
    default_nav2_params = os.path.join(wheelchair_navigation_dir, 'config', 'nav2_params_ablation_lidar_only_v2.yaml')
    ...
    # ========================================================================
    # LOCALIZATION SYSTEM (cameras still launch but are unused by Nav2)
    # ========================================================================
    ...
    localization_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            'use_fused_scan': 'false',   # ABLATION: AMCL uses /scan_filtered
            'rviz': 'false',
        }.items()
    )
    ...
    topic_data_logger = Node(
        ...
        parameters=[
            ...
            {'file_prefix': 'ablation_lidar_only_log'},
        ],
    )
    ...
    rviz_node = TimerAction(
        period=30.0,
        actions=[
            LogInfo(msg='[RVIZ] Starting RViz (ABLATION: LiDAR-only)...'),
            Node(
                package='rviz2',
                executable='rviz2',
                name='rviz2',
    ...
    nav2_startup = TimerAction(
        period=28.0,
        actions=[
            LogInfo(msg='[NAV2] Starting navigation stack (ABLATION: LiDAR-only)...'),
            controller_server,
            planner_server,
            behavior_server,
            bt_navigator,
    ...
    ready_message = TimerAction(
        ...
        actions=[
            LogInfo(msg='=' * 70),
            LogInfo(msg='  ABLATION: LIDAR-ONLY NAVIGATION - SYSTEM READY'),
            LogInfo(msg='=' * 70),
            LogInfo(msg='  Config: NO STVL, NO PRISM scan fusion'),
            LogInfo(msg='  AMCL: /scan_lidar (footprint-filtered LiDAR only)'),
            LogInfo(msg='  Local costmap: obstacle_layer + inflation (no cameras)'),
            LogInfo(msg='  Map: lidar-only SLAM map'),
    ...
    bag_output_dir = os.path.join(
        ws_root, 'rosbags', 'ablation_lidar_only_' + datetime.now().strftime('%Y%m%d_%H%M%S')
    )
